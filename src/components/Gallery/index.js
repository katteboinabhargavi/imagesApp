...
class Gallery extends Component {
  ...
  getFilteredImages = () => {
    ...
    const filteredImages = imagesList.filter(
      each => each.id === activeThumbnailId,
    )
  ...
  render() {
    ...
    return (
      <div className="bg-container">
        <ul>
          {filteredImages.map(details => (
            <li key={details.id} className="list">
              <img
        ...
        <ul className="bg-container-2">
          {imagesList.map(thumbnail => (
            <ThumbnailItem
              key={thumbnail.id}
              details={thumbnail}
              updateThumbnail={this.updateThumbnail}
              isActive={activeThumbnailId === thumbnail.Id}
            />
          ))}
        </ul>
      </div>
